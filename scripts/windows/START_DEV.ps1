...
if ($proxyRunning) {
    ...
    $proxyPath = "$ProjectRoot\bin\cloud-sql-proxy-win.exe"
    if (-not (Test-Path $proxyPath)) {
        Write-Host "[ERROR] Proxy binary not found at $proxyPath" -ForegroundColor Red
        exit 1
    }

    Write-Host "[STARTING] Cloud SQL Proxy..." -ForegroundColor Yellow
    Start-Process -FilePath $proxyPath -ArgumentList "einharjer-valhalla:us-east5:first-contact-db" -WindowStyle Hidden
